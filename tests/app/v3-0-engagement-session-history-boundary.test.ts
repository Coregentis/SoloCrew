import assert from "node:assert/strict";
import test from "node:test";

import {
  createEngagementSessionHistoryPageModel,
} from "../../app/shell/create-engagement-session-history-page-model.ts";
import {
  ENGAGEMENT_SESSION_HISTORY_BOUNDARY_FLAG_NAMES,
} from "../../app/engagement/engagement-session-history-contract.ts";
import {
  create_engagement_session_history_from_packet,
} from "../../app/engagement/engagement-session-history-workflow.ts";
import {
  createFounderReviewPacketFixture,
} from "../../projection/fixtures/v3-0-founder-review-packet-fixture.ts";
import {
  createEngagementSessionHistoryFixture,
} from "../../projection/fixtures/v3-0-engagement-session-history-fixture.ts";

const FORBIDDEN_FIELD_NAMES = [
  "payment_processor",
  "checkout",
  "subscription",
  "automated_billing",
  "crm",
  "email_dispatch",
  "public_publishing",
  "testimonial_publish",
  "external_analytics",
  "llm_call",
  "model_call",
  "agent_dispatch",
  "tool_invocation",
  "saas_sharing",
  "customer_account",
  "automatic_conversion",
  "provider_dispatch",
  "channel_dispatch",
  "marketplace",
  "autonomous_execution",
  "package_publish",
  "npm_publish",
  "mplp_certification",
  "mplp_endorsement",
  "persistence_adapter",
  "database_ref",
  "file_export_path",
  "route_url",
  "file_system_write",
  "database_storage",
  "cloud_sync",
] as const;

function collect_keys(value: unknown, keys = new Set<string>()): Set<string> {
  if (Array.isArray(value)) {
    for (const item of value) {
      collect_keys(item, keys);
    }

    return keys;
  }

  if (value !== null && typeof value === "object") {
    for (const [key, nested] of Object.entries(value)) {
      keys.add(key);
      collect_keys(nested, keys);
    }
  }

  return keys;
}

test("[v3.0] engagement session history keeps every boundary flag true", () => {
  const fixture = createEngagementSessionHistoryFixture();

  for (const bounded of [
    fixture.history_result,
    fixture.ledger,
    ...fixture.entries,
    fixture.export_package,
  ]) {
    for (const flag of ENGAGEMENT_SESSION_HISTORY_BOUNDARY_FLAG_NAMES) {
      assert.equal(bounded.boundary_flags[flag], true);
    }
  }
});

test("[v3.0] engagement session history exposes no forbidden positive fields", () => {
  const packet_fixture = createFounderReviewPacketFixture();
  const result = create_engagement_session_history_from_packet({
    ledger_id: "engagement-history-ledger-boundary",
    export_id: "engagement-session-export-boundary",
    created_at: "2026-04-30T15:45:00.000Z",
    packet_result: packet_fixture.packet_result,
  });
  const page_model = createEngagementSessionHistoryPageModel({
    result,
    generated_at: "2026-04-30T15:50:00.000Z",
  });
  const keys = collect_keys({ result, page_model });

  for (const forbidden_field of FORBIDDEN_FIELD_NAMES) {
    assert.equal(keys.has(forbidden_field), false, forbidden_field);
  }

  assert.equal(keys.has("no_file_system_write"), true);
  assert.equal(keys.has("no_database_storage"), true);
  assert.equal(keys.has("no_persistence_adapter"), true);
  assert.equal(keys.has("no_file_export_path"), true);
  assert.equal(keys.has("no_cloud_sync"), true);
});

test("[v3.0] engagement session history payload has no storage route cloud sync or execution payload", () => {
  const fixture = createEngagementSessionHistoryFixture();
  const payload = JSON.stringify(fixture);

  assert.doesNotMatch(payload, /route_url/i);
  assert.doesNotMatch(payload, /persistence_adapter_payload/i);
  assert.doesNotMatch(payload, /database_ref/i);
  assert.doesNotMatch(payload, /file_export_path_payload/i);
  assert.doesNotMatch(payload, /file_system_write_payload/i);
  assert.doesNotMatch(payload, /database_storage_payload/i);
  assert.doesNotMatch(payload, /cloud_sync_payload/i);
  assert.doesNotMatch(payload, /payment_processor/i);
  assert.doesNotMatch(payload, /checkout_url/i);
  assert.doesNotMatch(payload, /subscription_id/i);
  assert.doesNotMatch(payload, /crm_object_id/i);
  assert.doesNotMatch(payload, /email_dispatch_payload/i);
  assert.doesNotMatch(payload, /public_publishing_payload/i);
  assert.doesNotMatch(payload, /llm_execution_prompt/i);
  assert.doesNotMatch(payload, /model_call_payload/i);
  assert.doesNotMatch(payload, /agent_dispatch_payload/i);
  assert.doesNotMatch(payload, /tool_invocation_payload/i);
  assert.doesNotMatch(payload, /saas_share_url/i);
  assert.doesNotMatch(payload, /autonomous_execution_enabled/i);
  assert.doesNotMatch(payload, /raw_runtime_private_payload/i);
});
