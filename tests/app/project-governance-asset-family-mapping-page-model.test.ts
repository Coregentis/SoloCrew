...
import {
  createProjectGovernanceAssetFamilyMappingPageModel,
} from "../../app/shell/create-project-governance-asset-family-mapping-page-model.ts";
import {
  V2_STARTER_CELL_IDS,
...
function create_page_model(founder_request: string) {
  const proposal = createSecretaryRoutingProposal({
    request_id: "request-project-governance-page-001",
    founder_request,
  ...
  const directive = createManagementDirective({
    directive_id: "directive-project-governance-page-001",
  ...
  return createProjectGovernanceAssetFamilyMappingPageModel({
    mapping_id: "project-governance-page-mapping-001",
    source_preview: createCellCEOAssemblyPlanPreview({
      preview_id: "assembly-preview-project-governance-page-001",
...
test("[project governance mapping page model] wraps mapping as review-only surface", () => {
  const page_model = create_page_model(
    "Review TracePilot repo architecture and release drift."
  );

  assert.equal(page_model.page_title, "Project Governance Asset-Family Mapping");
...
test("[project governance mapping page model] exposes allowed review actions and forbidden controls", () => {
  const page_model = create_page_model(
    "Review TracePilot repo architecture and release drift."
  );
...
test("[project governance mapping page model] non-development Cell stays not applicable", () => {
  const page_model = create_page_model(
  ...
  assert.equal(page_model.mapping.external_product_modeled_as_cell, false);
  assert.equal(
    page_model.mapping.external_product_integration_implemented,
    false
  );
...
test("[project governance mapping page model] serialized output does not create TracePilot execution", () => {
  const page_model = create_page_model(
    "Review TracePilot repo architecture and release drift."
  );
